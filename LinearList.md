---
title: 线性表
date: 2025-03-17 00:41:13
updated:
type:
comments:
description: 数据结构与算法——线性表
keywords:
top_img:
mathjax: true
katex:
...
![alt text](LinearList/顺序表查找的ASL.png)
...
2. 则期望为：
![alt text](LinearList/顺序表插入的期望.png)
...
2. 共有 n - i - 1 个元素需要移动
3. 则期望为：
![alt text](LinearList/顺序表删除的期望.png)
***删除一个元素的平均时间复杂度为$O(n)$***
...
### 在链表中插入新节点
![alt text](LinearList/单向链表的插入.png)
**步骤**：
1. `s -> next = p -> next`
2. `p -> next = s`
...
### 在链表中删除节点
![alt text](LinearList/单向链表的删除.png)
**步骤**：
1. `q = p -> next`
2. `p -> next = q -> next`
3. `delete q; q = null`
...
### 双向链表的删除操作
![alt text](LinearList/双向链表的删除.png)
